...
function register(bot, deps) {
  ...
  bot.action('menu:help', async (ctx) => {
    ...
    const help =
      ...
      '3. Жмите «🖼 Создать изображение» и пройдите 5 шагов: запрос → правило → стиль → формат → фон.\n' +
      '4. Или «🛠 Загрузить и изменить»: исходник → опционально картинка для встройки → описание изменений → правило.\n\n' +
      '5. «🧩 Разобрать макет [BETA]» — скриншот UI/UX: распознавание, список, подтверждение, затем генерация с отправкой каждого ассета по готовности.\n\n' +
      'Под сгенерированной картинкой будут кнопки: «Изменить», «Объединить с картинкой», «Перегенерировать», «Генерировать новое».';
